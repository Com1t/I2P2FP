...
const int offsetX = 10, offsetY =600;
...
Menu::Menu()
{
    ...
    {
        ALGIF_ANIMATION *gif;
        sprintf(filename, "./Role/%s.gif", RoleStatus[i]);

        gif = algif_load_animation(filename);
    } */

    //ALGIF_ANIMATION *gif = algif_load_animation("./Role/normal.gif");
...
Menu::Draw()
{
    ...
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), offsetX + 2*al_get_bitmap_width(love), 560, 0, buffer);

    sprintf(buffer, "%d", HealthPoint);
    al_draw_bitmap(love, 10*offsetX, 560, 0);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), 10*offsetX + 2*al_get_bitmap_width(love), 560, 0, buffer);

    sprintf(buffer, "Coin: %d", Coin);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), 20*offsetX + 2*al_get_bitmap_width(love), 560, 0, buffer);

    sprintf(buffer, "Score: %d", Score);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255), 30* offsetX + 2*al_get_bitmap_width(love), 560, 0, buffer);

    for(int i=0; i < Num_TowerType; i++)
    {
        int pos_x = offsetX + (ThumbWidth + gapX) * (i % 10);
        int pos_y = offsetY + (ThumbHeight + gapY) * (i / 10);
        ...
        if(!Enough_Coin(i))
            ...
            al_draw_rectangle(pos_x, pos_y, pos_x + ThumbWidth, pos_y + ThumbHeight, al_map_rgb(0, 0, 255), 0);
        else
